...
Output (normalized):
  id, title, donor, url, deadline (YYYY-MM-DD), published_date (YYYY-MM-DD),
  status, themes (canonical list), country_scope (list[str]),
  amount_min, amount_max, currency, source_tags

Also: de-dup, cleanup titles, optional filters (future_only, require_deadline),
and Slack-friendly formatter (see aggregator).
"""
